...
public class MapManager {
    ...
    public void setFirstSelection(Player player, Location loc) {
        ...
        int y = (int) loc.getY();
        int z = (int) loc.getZ();
        player.sendMessage(ChatColor.GREEN + "[Pixel Mystery] Première sélection : " + x + ", " + y + ", " + z);
    }
    ...
    public void setSecondSelection(Player player, Location loc) {
        ...
        int y = (int) loc.getY();
        int z = (int) loc.getZ();
        player.sendMessage(ChatColor.GREEN + "[Pixel Mystery] Deuxième sélection : " + x + ", " + y + ", " + z);
    }
    ...
    public boolean create(Player player, String mapName) {
        ...
        if (loc1 == null || loc2 == null || loc1.getWorld() != loc2.getWorld()) {
            player.sendMessage(ChatColor.RED + "[Pixel Mystery] Sélections invalides.");
            return false;
        }

        if (config.get(mapName) != null) {
            player.sendMessage(ChatColor.RED + "[Pixel Mystery] Une map avec le même nom existe déjà.");
            return false;
        }
    ...
    public boolean delete(Player player, String mapName) {
        if (config.get(mapName) == null) {
            player.sendMessage(ChatColor.RED + "[Pixel Mystery] Cette map n'existe pas.");
            return false;
        }
